...
const markAllMatches = function (editor: Editor, currentSearchState: Cell<SearchState>, pattern: Pattern, inSelection: boolean) {
  ...
  if (inSelection) {
    return FindReplaceText.findAndMarkInSelection(editor.dom, pattern, editor.selection, marker);
  } else {
    return FindReplaceText.findAndMark(editor.dom, pattern, node, marker);
  }
};
